...
本课程是 ACL 三部曲的最后一部分，专注于性能优化、高级应用和生产环境部署实践。

**学习目标**:
- 掌握 ACL 性能优化技巧（批量分类、NUMA 感知、规则排序）
- 学习多分类器应用和动态规则更新
- 解决常见问题和选择合适的查找方法

**前置知识**: 完成 [Lesson 14-1](lesson14-1-acl-basics.md) 和 [Lesson 14-2](lesson14-2-acl-practice.md)
...
**性能优化技巧：**
- 批量分类（32-64 个数据包）可提升吞吐量 10-20 倍
- 常用规则设置高优先级，减少平均查找时间
- NUMA 感知分配，避免跨节点访问导致性能下降
